...
fn test_project_create_get_list_delete() {
    ...
    let id = repo
        .create(&brain_backend::project::NewProject {
...
fn test_vault_init_unlock_store_get() {
    ...
    let meta = vault
        .get_credential_metadata("api-key", "global", None)
        .unwrap()
        .unwrap();
...
fn test_run_lifecycle_with_events() {
    ...
    let run_id = run_repo
        .create(&brain_backend::run::NewRun {
    ...
    let event = event_store
        .insert_event(run_id, "message", r#"{"text":"hello"}"#)
        .unwrap();
    assert!(event.id > 0);
    ...
    run_repo
        .transition(
            run_id,
            brain_backend::run::state::RunStatus::Running,
            Some("started"),
            None,
            None,
        )
    ...
    run_repo
        .transition(
            run_id,
            brain_backend::run::state::RunStatus::Completed,
            None,
            Some("done"),
            None,
        )
...
fn test_tool_invocations() {
    ...
    let tool_id = tool_repo
        .start(&brain_backend::run::tools::NewToolInvocation {
            run_id,
            event_id: None,
    ...
    tool_repo
        .complete(
            tool_id,
            &brain_backend::run::tools::ToolResult {
...
fn test_context_builder_assembly() {
    ...
    ctx_repo
        .upsert(run_id, "system_prompt", "You are helpful")
        .unwrap();
    ctx_repo
        .upsert(run_id, "tools_json", r#"[{"name":"search"}]"#)
        .unwrap();
...
fn test_workspace_read_write_list() {
    ...
    workspace
        .write_file(&uuid, "test.txt", b"hello world")
        .unwrap();
...
fn test_full_flow_project_run_memory() {
    ...
    let mem_id = mem_repo
        .insert(&brain_backend::memory::repository::NewMemory {
            project_id: Some(project_id),
            collection_id: 1,
            run_id: None,
    ...
    let memories = mem_repo
        .list_by_project(project_id, Some("episodic"), 10)
        .unwrap();
